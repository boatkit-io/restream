...
- `Stores` builds relay stores for each device. `game.NewRelayStores()` is generated by `@restream.store`; codegen hardcodes each source store's optional `GetMinimumAccessLevel` constant into the relay store factory.
