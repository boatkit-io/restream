...
- `Stores` builds relay stores for each device.
- `AuthenticateDevice` checks the device hello and returns the connection access level.
...
- `device.RPCHandler` forwards cloud viewer RPCs back to the connected device server.
